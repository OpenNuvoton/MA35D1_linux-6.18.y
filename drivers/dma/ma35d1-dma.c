...
static void ma35d1_set_dma_timeout(struct ma35d1_chan *ch)
{
	...
	if (ch->ch_num <= 7) {
		...
		val = readl(ch->base + PDMA_TOUTPSC1);
		val |= ((d->pcfg.timeout_prescaler & 0x7) << (4 * (ch->ch_num - 8)));
		writel(val, ch->base + PDMA_TOUTPSC1);
	}
	...
	val = readl(ch->base + PDMA_TOC + (4 * (ch->ch_num / 2)));
	val |= ((d->pcfg.timeout_counter & 0xffff) << (16 * (ch->ch_num % 2)));
	writel(val, ch->base + PDMA_TOC + (4 * (ch->ch_num / 2)));
...
static int ma35d1_slave_config(struct dma_chan *chan, struct dma_slave_config *cfg)
{
	struct ma35d1_chan *ch = to_ma35d1_dma_chan(chan);
...
static struct dma_async_tx_descriptor *ma35d1_prep_dma_memcpy(struct dma_chan *chan, dma_addr_t dst,
...
static struct dma_async_tx_descriptor *ma35d1_prep_slave_sg(struct dma_chan *chan,
							    ...
							    unsigned long tx_flags, void *context)
{
	...
	if (ch->cfg.peripheral_size != sizeof(struct ma35d1_peripheral)) {
		dev_err(chan2dev(chan),
			...
			ch->cfg.peripheral_size, sizeof(struct ma35d1_peripheral));
		return NULL;
	}
	...
	for_each_sg(sgl, sgent, sg_len, i) {
		d->sg[i].ctl =
		    (ch->runtime_ctrl |
		     PDMA_TXCNT(sg_dma_len(sgent) - 1UL) | PDMA_TBINTDIS | PDMA_OP_SCATTER);
...
static struct dma_async_tx_descriptor *ma35d1_prep_dma_cyclic(struct dma_chan *chan,
							      ...
							      unsigned long flags)
{
	...
	for (offset = 0, i = 0; offset < buf_len; offset += period_len, i++) {
		d->sg[i].ctl = (ch->runtime_ctrl |
				PDMA_TXCNT((period_len / ch->runtime_width) - 1) | PDMA_OP_SCATTER);
		if (dir == DMA_MEM_TO_DEV) {
...
static struct dma_chan *ma35d1_of_xlate(struct of_phandle_args *dma_spec, struct of_dma *of)
{
...
static enum dma_status ma35d1_tx_status(struct dma_chan *chan, dma_cookie_t cookie,
...
{
	struct ma35d1_dmadev *edma = container_of(dma_dev, struct ma35d1_dmadev, ddev);
...
static int ma35d1_probe(struct platform_device *pdev)
{
	...
	edma->ddev.dst_addr_widths = BIT(DMA_SLAVE_BUSWIDTH_4_BYTES) |
	    BIT(DMA_SLAVE_BUSWIDTH_2_BYTES) | BIT(DMA_SLAVE_BUSWIDTH_1_BYTE);
	edma->ddev.directions = BIT(DMA_MEM_TO_DEV) | BIT(DMA_DEV_TO_MEM) | BIT(DMA_MEM_TO_MEM);
	...
	ret = devm_request_irq(dev, irq, ma35d1_dma_interrupt, 0, pdev->name, edma);
	if (ret) {
