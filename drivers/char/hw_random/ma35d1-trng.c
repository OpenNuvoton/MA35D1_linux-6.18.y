...
/*--------------------------------------------------------------*/
/*  MA35D1 Series SoC TRNG registers                              */
/*--------------------------------------------------------------*/
#define CTRL			0x000
...
static inline void ma35d1_write_reg(struct ma35d1_trng *tdev, u32 val, u32 reg)
{
	writel_relaxed(val, tdev->reg_base + reg);
}

static inline u32 ma35d1_read_reg(struct ma35d1_trng *tdev, u32 reg)
{
...
static inline int ma35d1_trng_wait_busy_clear(struct ma35d1_trng *tdev)
{
	...
	while (ma35d1_read_reg(tdev, STAT) & STAT_BUSY) {
...
static int ma35d1_trng_issue_command(struct ma35d1_trng *tdev, int cmd)
{
	...
	ma35d1_write_reg(tdev, (ma35d1_read_reg(tdev, CTRL) & ~CTRL_CMD_MASK) |
			(cmd << CTRL_CMD_OFFSET), CTRL);

	timeout = jiffies + msecs_to_jiffies(TRNG_TIMEOUT);
	while (!(ma35d1_read_reg(tdev, ISTAT) & ISTAT_DONE)) {
		if (time_after(jiffies, timeout)) {
			pr_debug("TRNG command %d timeout! ISTAT = 0x%x, SMODE = 0x%x.\n",
				 cmd, ma35d1_read_reg(tdev, ISTAT),
				 ma35d1_read_reg(tdev, SMODE));
			return -EBUSY;
		}
	}
	return 0;
...
static int ma35d1_trng_gen_nonce(struct ma35d1_trng *tdev, uint32_t *nonce)
{
	...
	ma35d1_write_reg(tdev, ma35d1_read_reg(tdev, SMODE) | SMODE_NONCE, SMODE);

	if (ma35d1_read_reg(tdev, MODE) & MODE_SEC_ALG)
		loop = 3;
	else
		loop = 2;
	...
	for (i = 0; i < loop; i++) {
		...
		for (j = 0; j < 16; j++)
			ma35d1_write_reg(tdev, nonce[i * 16 + j], NPA_DATA(j));
...
static int ma35d1_trng_init(struct hwrng *rng)
{
	...
	if (ma35d1_read_reg(tdev, STAT) & (STAT_STARTUP_TEST_STUCK |
	...
	/* SELECT_ALG_AES_256 */
	ma35d1_write_reg(tdev, ma35d1_read_reg(tdev, MODE) | MODE_SEC_ALG, MODE);
...
static int ma35d1_trng_read(struct hwrng *rng, void *buf, size_t max, bool wait)
{
	struct ma35d1_trng *tdev = (struct ma35d1_trng *)rng->priv;
	u32 *data = buf;
	int i, err, retval = 0;
	...
	while (max >= 4) {
		...
		for (i = 0; i < 4; i++) {
			if (max < 4)
				break;
			*data = ma35d1_read_reg(tdev, RAND(i));
			pr_debug("%08x ", *data);
			data++;
			max -= 4;
			retval += 4;
...
static void ma35d1_trng_remove(struct platform_device *pdev)
{
}
