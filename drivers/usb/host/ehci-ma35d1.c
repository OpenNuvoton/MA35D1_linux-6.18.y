...
static void ehci_ma35d1_drv_remove(struct platform_device *pdev)
{
	struct usb_hcd *hcd = platform_get_drvdata(pdev);
...
#ifdef CONFIG_MA35_DPD_LOW_POWER
	regmap_read(ma35d1_ehci->sysregmap, REG_SYS_USBPMISCR, &reg);
	regmap_write(ma35d1_ehci->sysregmap, REG_SYS_USBPMISCR, (reg & ~0x20002));
#endif
